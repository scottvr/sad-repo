...
- **Pressure grid (2026-07-14): replay survives 3 of 4 threats.** Cramming refuted (`newest_alone_on_earlier` at the base floor everywhere); the conflict arm genuinely passes (conflicting shared words disambiguated by the domain-naming prompt, 0.20 → 0.98); but retention scales ~linearly with replay fraction (only full rehearsal saturates — joint training in disguise; pseudo-replay is the escape hatch) and the big family (24 facts) deflates the headline to 0.69. Capacity floor: ~4 dims/fact ≈ 0.7 retention, ~8 dims/fact saturates (dims grid: total dims is the resource, site/layer allocation barely matters); confirmed out of sample 2026-07-15 by the `big_k16` arm — 192 dims on the 24-fact family restores retention to 1.00 over 5 seeds. Paraphrase consistency still 0.0 in every arm — RETRACTED 2026-07-15: a state bug made `coherence_probe` measure the frozen base in every historical run (leakage was exactly 1/len(labels) everywhere). Fixed in `experiments.py` (composed-state probe + `coherence_base` floor; regression tests in `tests/test_coherence_state.py`); kill-criterion status is UNKNOWN until grids are re-run.
